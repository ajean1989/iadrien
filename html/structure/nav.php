<nav>
    <ul>
        <li><a href="bases_html.php">HTML - Bases</a></li>
        <li><a href="bases_css.php">CSS - Bases</a></li>
        <li><a href="animation_css.php">CSS - Animations</a></li>
        <li><a href="structure_html.php">HTML CSS - Structure - flex, ... </a></li>
        <li><a href="enrichissements_html.php">HTML CSS - Tableaux - Formulaire - AV - ...</a></li>
        <li><a href="sass.php">Sass</a></li>
        <li><a href="builder.php">NPM - Composer</a></li>
        <li><a href="bases_php.php">PHP - Bases</a></li>
        <li><a href="MVC.php">PHP - Le modèle MVC</li>
        <li><a href="PHP_POO.php">PHP - POO</li>
        <li><a href="parametrage_erreurs_securité.php">PHP - Paramétrages - Sécurité - API</a>
        <li><a href="MySQL.php">MySQL</a></li>
        <li><a href="bases_javascript.php">Javascript - Bases</a></li>
        <li><a href="js_objets.php">Javascript - Objets natifs - namespace, regex, string, number, math, array... </a></li>
        <li><a href="js_manipulation_de_page.php">Javascript - manipulation de pages</a></li>
        <li><a href="js_serveur.php">Javascript - Asynchrone - serveur - API</a></li>
        <li><a href="js_modules.php">Javascript - Modules - Webpack - Parcel</a></li>
        <li><a href="linuxserveur.php">Linux</a></li>
        <li><a href="reseaux.php">Réseaux</a></li>
        <li><a href="protocoles.php">Protocoles - HTTP</a></li>
        <li><a href="git.php">Git & Github</a></li>
        <li><a href="securite.php">Sécurité</a></li>
        <li><a href="SOLIDPattern.php">SOLID / Design Pattern</a></li>
        <li><a href="firefox.php">Firefox - Extensions - Outils</a></li>
        
    </ul>
</nav>